...
class ContentQualityAnalyzer {
  constructor() {
    this.targetKeywords = {
      ...
      tier3: ['免費線上點擊速度測試遊戲', '支援離線的點擊遊戲', 'TPS計算遊戲', '跨平台點擊遊戲', '響應式點擊遊戲']
    };
  ...
  analyzeMetaDescription(htmlContent) {
    const metaMatch = htmlContent.match(/<meta[^>]+name=['"](description|Description)['"]+[^>]+content=['"]([^'"]*)['"]/i);
    ...
    return { score: Math.max(0, score), description, length: description.length, keywordCount, issues };
  }
  ...
  analyzeInternalLinks(htmlContent) {
    ...
    const internalLinks = allLinks.filter(link => 
      !link.includes('http') || link.includes('haotool.github.io/clickfun')
    );
    ...
    const genericAnchors = anchorTexts.filter(anchor => {
      const text = anchor.replace(/<[^>]*>/g, '').trim().toLowerCase();
    ...
    return { score: Math.max(0, score), totalLinks: allLinks.length, internalLinks: internalLinks.length, issues };
  }
  ...
  analyzeImageOptimization(htmlContent) {
    ...
    return { score: Math.max(0, score), totalImages: images.length, missingAlt, missingLazyLoad, issues };
  }

  // 分析結構化數據
  analyzeStructuredData(htmlContent) {
    const jsonLdScripts = htmlContent.match(/<script[^>]*type=['"](application\/ld\+json|application\/json)['"]*[^>]*>[\s\S]*?<\/script>/gi) || [];
    const issues = [];
    let score = 100;
  ...
  generateMarkdownReport(report) {
    const timestamp = new Date(report.analysis_timestamp).toLocaleString('zh-TW', { timeZone: 'Asia/Taipei' });
    ...
    Object.keys(report.keyword_analysis).forEach(tier => {
      ...
      Object.keys(report.keyword_analysis[tier]).forEach(keyword => {
        ...
        const status = data.density >= 1 && data.density <= 2 ? '✅ 理想' : 
                      data.density < 0.5 ? '⚠️ 過低' : '❌ 過高';
